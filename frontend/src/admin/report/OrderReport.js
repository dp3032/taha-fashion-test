...
const OrderReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      const response = await axios.get(`${apiUrl}/all-report/report/monthly/order?month=${month}&year=${year}`,{
        headers: {
  ...
  const generatePDF = () => {
    const doc = new jsPDF();
    const header = ['No.', 'Name', 'Address', 'Product Details', 'Amount', 'Status', 'Order Date', 'Payment ID'];
    const columnWidths = [10, 20, 30, 30, 20, 20, 30, 30];
    ...
    doc.setFontSize(14);
    doc.text(`Monthly Order Report: ${months.find((m) => m.value === month)?.label} ${year}`, 10, 10);
    ...
    report.forEach((order, index) => {
      const rowData = [
        `${index + 1}`, // No.
        order.UserName, // Name
        `${order.address}, ${order.city}, ${order.state} - ${order.zipcode}`, // Address
        Array.isArray(order.products)
          ? order.products
              .map(
                (product) =>
                  `${product.product_name} (${product.selectedSize}) - ${product.quantity} x ${product.price}`
              )
              .join('\n')
          : 'No products', // Product Details
        String(order.totalAmount), // Amount
        order.order_status, // Status
        order.order_date, // Order Date
        Array.isArray(order.paymentDetails) && order.paymentDetails.length > 0
          ? `${order.paymentDetails[0].payment_id}\n${order.paymentDetails[0].order_id}`
          : 'N/A', // Payment ID
      ];
    ...
    // Save the PDF
    doc.save(`Order_Report_${month}_${year}.pdf`);
  };
    ...
    <LeftSide />
      <div className="bord">
        ...
        <div className="user-report">
          <h2 className="user-report__title">Order Report</h2>
          ...
          {loading ? (
            ...
            report.length > 0 && (
              <table className="user-report__table">
                ...
                <tbody>
                  {report.slice().reverse().map((order, index) => (
                    <tr key={order._id}>
                        <td> {index + 1} </td>
                        ...
                        <td>
                            {Array.isArray(order.paymentDetails) ? (
                                <ul className="product-items">
                                        {order.paymentDetails.map((payment, i) => (
                                            <>
                                                <p className="order-item">{payment.payment_id} </p>
                                                <p className="order-item">{payment.order_id}</p>
...
export default React.memo(OrderReport);
