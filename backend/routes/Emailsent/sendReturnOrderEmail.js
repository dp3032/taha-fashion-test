...
const sendReturnOrderEmail = async (orderDetails, userEmail) => {
  ...
  const mailOptions = {
    ...
    to: userEmail,
    subject: `Return Order Details with TAHA FASHION`,
    html: `
      <div style="font-family: Arial, sans-serif; color: #333; padding: 20px; background-color: #f7f7f7; border-radius: 10px; max-width: 600px; margin: auto;">
        <div style="background-color: #f2a365; padding: 15px; text-align: center; color: white; border-radius: 10px 10px 0 0;">
          <h2 style="margin: 0; font-size: 24px;">Return Order Details</h2>
          <p style="margin: 0;">Return ID: <strong>${orderDetails._id}</strong></p>
        </div>

        <div style="padding: 20px;">
          <h3 style="color: #ff7043;">Return Information</h3>
          <p><strong>Order ID:</strong> ${orderDetails.order_id}</p>
          <p><strong>Order Date:</strong> ${orderDetails.order_date}</p>
          <p><strong>Return Date:</strong> ${orderDetails.Retunorder_date}</p>
          <p><strong>Total Amount:</strong> ${orderDetails.totalAmount}</p>
          <p><strong>Return Status:</strong> ${orderDetails.Retunorder_status}</p>
          <p><strong>Reason:</strong> ${orderDetails.reason}</p>

        <h3 style="color: #ff7043;">User Details</h3>
          ...
          <h3 style="color: #ff7043;">Product Details</h3>
          ...
          <h3 style="color: #ff7043;">Payment Details</h3>
            <ul style="list-style-type: none; padding-left: 0;">
          ...
          <div style="text-align: center; padding: 20px;">
            <a href="${process.env.FROAPI}" style="background-color: #ff7043; color: white; padding: 10px 20px; text-decoration: none; font-size: 16px; border-radius: 5px;">
              Shop Again
            </a>
          </div>

          <p style="font-size: 16px; line-height: 1.6; text-align: center;">Thank you for shopping with TAHA FASHION!</p>
        </div>
...
module.exports = sendReturnOrderEmail;
